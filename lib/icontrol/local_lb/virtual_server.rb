...
module IControl # :nodoc:
  module LocalLB # :nodoc:
    ...
    #
    #  # To retreive every virtual server
    #  every_virtual_server =  IControl::LocalLB::VirtualServer.find(:all)
    #  
    #
    ...
    class VirtualServer
      ...
      ##
      # :method: fallback_persistence_profile
      ...
      def persistence_record(mode) # :nodoc:
      ...
      ##
      # This method sets the translate_address_state to STATE_ENABLED
      def enable_address_translation!
        translate_address_state = IControl::Common::EnabledState::STATE_ENABLED
      end

      ##
      # This method sets the translate_address_state to STATE_DISABLED
      ...
      # Sets the cmp_enabled_state to STATE_DISABLED
      def disable_cmp!
      ...
      # Sets the VLANs this virtual server belongs to 
      # The +vlan+ parameter can be an instance of <tt>IControl::Common::VLANFilterList</tt> 
      # or an array with the names of the Vlans the virtual server is enabled on. If a +vlan+ is nil then the virtual_server will be in every vlan
      #
      # === Example
      #
      #  # Setting Vlans by means of an instance of IControl::Common::VLANFilterList
      #  virtual_server.vlan = IControl::Common::VLANFilterList.new(:state => IControl::Common::EnabledState::STATE_ENABLED,
      #                                                             :vlans => ["vlan1","vlan2"])
      #
      #  # Setting them using an array of names
      #  virtual_server.vlan = ["vlan1","vlan2"]
      #
      def vlan=(vlan)
        vlan = IControl::Common::VLANFilterList.new(:state => IControl::Common::EnabledState::STATE_ENABLED,:vlans => vlan) if vlan.is_a? Array
        count=0;
        vlans = {}
